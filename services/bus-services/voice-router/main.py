...
class VoiceRouter(BusServiceWrapper):
    ...
    def __init__(self):
        ...
        # Start session cleanup (will be started in main())
        self.session_cleanup_task = None
...
async def main():
    ...
    voice_router.session_cleanup_task = asyncio.create_task(voice_router._cleanup_expired_sessions())

    # Start API server
    try:
        import uvicorn
        config = uvicorn.Config(voice_router.api.app, host="0.0.0.0", port=8007)
