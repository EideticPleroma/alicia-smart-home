...
class DeviceManager(BusServiceWrapper):
    ...
    def __init__(self):
        ...
        # Start background tasks (will be started in main())
        self.command_queue_task = None
    ...
    def _handle_device_registration(self, message: Dict[str, Any]):
        ...
        try:
            # Message is already the payload from process_message
...
async def main():
    ...
    device_manager.command_queue_task = asyncio.create_task(device_manager._process_command_queue())
    device_manager.device_monitor_task = asyncio.create_task(device_manager._monitor_device_status())

    # Start API server
    try:
        import uvicorn
        config = uvicorn.Config(device_manager.api.app, host="0.0.0.0", port=8008)
