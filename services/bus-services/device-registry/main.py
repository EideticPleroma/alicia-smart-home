...
class DeviceRegistry(BusServiceWrapper):
    ...
    def __init__(self):
        ...
        self.db_path = str(self.registry_data_path / "devices.db")
...
async def main():
    ...
    registry_service.device_cleanup_task = asyncio.create_task(registry_service._device_cleanup_task())
    registry_service.heartbeat_monitor_task = asyncio.create_task(registry_service._heartbeat_monitor_task())
    registry_service.status_update_task = asyncio.create_task(registry_service._status_update_task())

    # Start API server
    try:
        import uvicorn
        config = uvicorn.Config(registry_service.api.app, host="0.0.0.0", port=8010)
